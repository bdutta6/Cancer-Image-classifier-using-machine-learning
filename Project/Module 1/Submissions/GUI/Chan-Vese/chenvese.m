...
im_scale_lim = 1024;
s = im_scale_lim./min(size(I,1),size(I,2)); % resize scale
if s<1
    I = imresize(I,s);
end
...
switch lower(method)
    ...
    case 'multiphase'
        layer = size(I,3);
        if size(I,1)*size(I,2)>im_scale_lim^2
            s = im_scale_lim./min(size(I,1),size(I,2)); % resize scale
            I = imresize(I,s);
            mask = imresize(mask,s);
        end
...
%         close h
    case 'multiphase'
        ...
        phi2=bwdist(mask2)-bwdist(1-mask2)+im2double(mask2)-.5;%Get phi2 from the initial mask 2
...
%         subplot(2,2,2);
%         hold on
%         contour(flipud(mask1),'r','LineWidth',2.5);
%         contour(flipud(mask1),'x','LineWidth',1);
%         contour(flipud(mask2),'g','LineWidth',2.5);
%         contour(flipud(mask2),'x','LineWidth',1);
...
%                 showphi(I, new, n);
                ...
                % seg = seg11+2*seg12+3*seg21+4*seg22;
                seg = 2*seg11+seg12+3*seg21+4*seg22;
                seg = imerode(seg,se);
...
%         showphi(I, phi, n);
        ...
        %         %
        %         aa1 = imerode(seg11,se);
        %         aa2 = imerode(seg12,se); % nuclei
        %         aa3 = imerode(seg21,se); % bg
        %         aa4 = imerode(seg22,se);
