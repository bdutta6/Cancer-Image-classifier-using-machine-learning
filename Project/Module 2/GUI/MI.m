function [selected_features,features_idx,weights] = MI(features,labels,Q)
...
[weights,features_idx] = sort(I,'descend');

for val=1:length(features_idx)
    selected_features(:,val) = features(:,features_idx(val));
end
